...
def group_collide(grp, other_obj):
    global collision_group, explosion_group
    count_collision = 0
    for obj in set(grp):
...
def group_group_collide(grp_1, grp_2):
    count_collisions = 0
    for elem_grp_1 in set(grp_1):
        collisions = group_collide(grp_2, elem_grp_1)
        if collisions > 0:
            grp_1.remove(elem_grp_1)
            count_collisions += collisions
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            explosion_index = [EXPLOSION_DIM [0], (self.age // EXPLOSION_DIM[0]) % EXPLOSION_DIM[1]]
    ...
    def collide(self, other_obj):
        # detects collision
        distance = math.sqrt(math.pow(self.get_position()[0]-other_obj.get_position()[0], 2) + math.pow(self.get_position()[1]-other_obj.get_position()[1], 2));
...
def draw(canvas):
    global time, started, rock_group, lives, missile_group, score, soundtrack, explosion_group
    ...
    my_ship.update()
    #a_rock.update()
    # call sprite_group on every rock in rock_group
    ...
    if lives <=0:
        ...
        started = False
        soundtrack.pause()
        ## Can stop the thrust and other things as well if needed
        ##my_ship.set_thrust(False)
...
def rock_spawner():
    ...
    if len(rock_group) < 12 and started:
        ...
        if  div > 0:
            velocity_fact = div
        rock_vel = [random.random() * velocity_fact - .3, random.random() * velocity_fact - .3]
        rock_avel = random.random() * .2 - .1
        a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
        
        # ignore rock if it collides with the ship
